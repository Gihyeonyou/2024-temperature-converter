...
class Converter:...
    ...
    def open_history(self):
        history_window = HistoryExport(self, self.all_calculations)
...
class HistoryExport:
    ...
    def __init__(self, partner, calc_list):
       ...
       # Set filename variable
        ...
        if num_calcs > max_calcs:
            ...
            showing_all = "Below is your calculations history."
    ...
    def get_calc_string(self, var_calculations):
        ...
        num_calcs = len(var_calculations)
        stop = min(num_calcs, max_calcs)

        # iterate to all but last item,
        # adding item and line break to calculation string
        for item in range(stop - 1):
            calc_string += var_calculations[num_calcs - item - 1]
            calc_string += "\n"

        # add final item without an extra linebreak
        # ie: last item on list will be fifth from the end!
        calc_string += var_calculations[num_calcs - max_calcs]
    ...
    def close_history(self, partner):...
        self.history_box.destroy()  # Close the Toplevel window
        partner.to_history_button.config(state=NORMAL)  # Enable the "History / Export" button
